export const createTokenEvent =
  'event TokenCreated(address by,uint256 tokenId,address[] creators,uint256[] creatorsRoyalty,uint8 status,bytes32 digest,uint8 hashFunction,uint8 size)';

export const createBidEvent = 'event NewBid(address by, uint256 tokenId, uint256 amount);'

export const contractAddress = '0xAf266B3D45B11D8B3f28dc2427745c3970B0368C';
